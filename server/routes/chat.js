    ...
    if (cleaned && cleaned !== 'SKIP' && cleaned.length > 5 && cleaned.length < 200) {
      ...
      if (agent) {
        generateMemoryArt(agent, cleaned).then(art => {
          if (art) {
            db.prepare('UPDATE agent_memories SET art_url = ?, art_ipfs_cid = ?, art_prompt = ?, nft_token_id = ?, nft_contract = ?, auction_status = ? WHERE id = ?')
              .run(art.imageUrl, art.ipfsCid, art.prompt, art.nftTokenId, art.nftContract, art.auctionStatus, result.lastInsertRowid);
          }
        }).catch(() => {});
      }
